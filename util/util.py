...
def cal_mask_given_mask_thred(img, mask, patch_size, stride, mask_thred):
    ...
    for i in range(N):
        ...
        #해당 좌표가 mask픽셀인지 아닌지 출력. 1이면 masking, 0이면 non-masking.
        
        if torch.sum(mask_tmp) < mask_thred: # 마스크 판단 임계값을 넘지 않는다 == non-mask영역 
            nonmask_point_idx_all[tmp_non_mask_idx] = i #nonmask_point_idx_all에 그 인덱스 하나씩 넣음.
            tmp_non_mask_idx += 1
        else:
            mask_point_idx_all[tmp_mask_idx] = i
            tmp_mask_idx += 1
            #위와 동일하고 flag는 0으로 초기화 되어있지만 mask 픽셀은 1로 체크한다.
            #offsets_temp_vec또한 1024 배열이 0으로 초기화되어있지만 mask 픽셀은 -1로 체크한다.
            flag[i] = 1
            offsets_tmp_vec[i] = -1
    ...
    # nonmask_point_idx_all와 mask_point_idx_all은 1024 프레임에 맞는거 넣다보니 뒷단이 당연히 비어있음.
    #이를 딱 맞게 맞춰서 잘라내기 위해 narrow 사용. narrow를 통해 뒤 비어있는 공간을 없앰.
    ...
    for i in range(N):
        ...
        #0에서부터 i까지 -1의 개수를 다 더한다. (당연히 i가 커질수록 점점 늘어남)
        if flag[i] == 1:
            offset_value = offset_value + 1
            #근데 flag가 1이면, 즉 mask pixel이면 offset_value를 +1한다.
#-> 연속으로 지나온 마스크 영역의 개수를 나타낸다.
# offset_value = ex) 0 1 1 1 0 0 0 1 => 0 0 1 2 3 3 3 3 
        flatten_offsets_all[i+offset_value] = -offset_value
    flatten_offsets = flatten_offsets_all.narrow(0, 0, non_mask_num)
    #i늘어날 때 offset도 늘어나면 값 유지됨.(offset은 연속 마스크면 늘어남)
